...
export default function TodoList() {
  ...
  const handleEdit = async (id: string) => {
    ...
    try {
      await updateTodo({ id, updates: { title: newTitle } });
    } catch (err) {
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      ...
                      disabled={isUpdating}
                      className="px-4 py-2 text-sm font-medium bg-blue-500 text-white rounded-lg hover:bg-blue-600 active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 shadow-sm"
                    >
                      {isUpdating ? (
                    ...
                    <button
                      ...
                      disabled={isDeleting}
                      className="px-4 py-2 text-sm font-medium bg-red-500 text-white rounded-lg hover:bg-red-600 active:scale-95 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 shadow-sm"
                    >
                      {isDeleting ? (
                        <span className="flex items-center gap-2">
                          <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
